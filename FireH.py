...
def main():
        ...
        epilog=textwrap.dedent('''\
                                target commands:
                                    magic                           causes the magic function in the target_TargetName.py file to be called -
                                                                    on cph1933 this will activate the secure boot exploit and the device will 
                                ...
                                fw commands:
                                    ...
                                    poke addr val                   writes 64 bytes of data specified by the hex value val at address addr
                                    
                                    
                                Usage examples:
                                    python firehorse.py -s -c COM19 -t cph1933 target magic
                                    python firehorse.py -c COM19 -t cph1933 fw hello
                                    python firehorse.py -c COM19 -t cph1933 fw peek 0x120000 0x10
                                '''))
    ...
    parser.add_argument('-t', '--target', dest='target_name',
                        help='Specify target, can be one of the following: cph1933 | angler | ugglite | mido | cheeseburger', required=True)
    ...
    try:
        ...
        E("unknown target formatter_class")
        sys.exit(1)
